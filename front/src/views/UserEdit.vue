...
export default {
    ...
    async modify() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          await this.$router.push({ name: 'User', params: { username: this.username } })
        }
  ...
  async created() {
    if (!this.user.admin && this.user.username !== this.$route.params.username)
      return await this.$router.push({ name: 'User', params: { username: this.$route.params.username } })
    const requestSettings = {
      headers: {
    ...
    try {
      ...
      if (res.ok) {
        ...
        this.pfp = json.pfp
        json.admin == 0 ? this.admin = false : this.admin = true
      }
      else {
